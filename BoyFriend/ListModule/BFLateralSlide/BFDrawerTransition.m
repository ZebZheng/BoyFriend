...
@interface BFDrawerTransition ()

@property (nonatomic,weak) BFLateralSlideConfiguration *configuration;

@end

@implementation BFDrawerTransition
{
    BFDrawerTransitiontype _TransitionType;
    BFDrawerAnimationType _animationType;
...
- (instancetype)initWithTransitionType:(BFDrawerTransitiontype)transitionType animationType:(BFDrawerAnimationType)animationType configuration:(BFLateralSlideConfiguration *)configuration {
    if (self = [super init]) {
        ...
        if (_TransitionType == BFDrawerTransitiontypeHidden)
...
- (NSTimeInterval)transitionDuration:(nullable id <UIViewControllerContextTransitioning>)transitionContext {
    return _TransitionType == BFDrawerTransitiontypeShow ? self.configuration.showAnimDuration : self.configuration.HiddenAnimDuration;
}

- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext {
    switch (_TransitionType) {
        case BFDrawerTransitiontypeShow:
            [self animationViewShow:transitionContext];
            break;
        case BFDrawerTransitiontypeHidden:
...
- (void)animationViewShow:(id <UIViewControllerContextTransitioning>)transitionContext {
    if (_animationType == BFDrawerAnimationTypeDefault) {
        [self defaultAnimationWithContext:transitionContext];
    }else if (_animationType == BFDrawerAnimationTypeMask) {
...
- (void)animationViewHidden:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    BFMaskView *maskView = [BFMaskView shareInstance];
...
- (void)defaultAnimationWithContext:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    CGFloat x = - width / 2;
    CGFloat ret = 1;
    if (self.configuration.direction == BFDrawerTransitionFromRight) {
...
@implementation BFMaskView

static BFMaskView *bf_shareInstance = nil;
static dispatch_once_t bf_onceToken;
+ (instancetype)shareInstance {
    dispatch_once(&bf_onceToken, ^{
        bf_shareInstance = [[BFMaskView alloc] init];
    });
    return bf_shareInstance;
}

+ (void)releaseInstance{
    [bf_shareInstance removeFromSuperview];
    bf_onceToken = 0;
    bf_shareInstance = nil;
}
...
- (void)singleTap {
    [[NSNotificationCenter defaultCenter] postNotificationName:BFLateralSlideTapNoticationKey object:self];
}

- (void)handleGesture:(UIPanGestureRecognizer *)pan {
    [[NSNotificationCenter defaultCenter] postNotificationName:BFLateralSlidePanNoticationKey object:pan];
}
...
NSString *const BFLateralSlideMaskViewKey = @"BFLateralSlideMaskViewKey";
NSString *const BFLateralSlideAnimatorKey = @"BFLateralSlideAnimatorKey";
NSString *const BFLateralSlideInterativeKey = @"BFLateralSlideInterativeKey";

NSString *const BFLateralSlidePanNoticationKey = @"BFLateralSlidePanNoticationKey";
NSString *const BFLateralSlideTapNoticationKey = @"BFLateralSlideTapNoticationKey";

NSString *const BFLateralSlideDirectionKey = @"BFLateralSlideDirectionKey";
