...
+ (NSUInteger)br_getDaysInYear2:(NSInteger)year month:(NSInteger)month {
    NSDate *date = [NSDate br_dateFromString:[NSString stringWithFormat:@"%@-%@", @(year), @(month)] dateFormat:@"yyyy-MM"];
    // 指定日历的算法(这里按公历)
...
- (NSComparisonResult)br_compare:(NSDate *)targetDate format:(NSString *)format {
    NSString *dateString1 = [NSDate br_stringFromDate:self dateFormat:format];
    NSString *dateString2 = [NSDate br_stringFromDate:targetDate dateFormat:format];
    NSDate *date1 = [NSDate br_dateFromString:dateString1 dateFormat:format];
    NSDate *date2 = [NSDate br_dateFromString:dateString2 dateFormat:format];
