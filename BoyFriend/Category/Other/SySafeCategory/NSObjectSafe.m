...
if (!(condition)){ SFLog(__FILE__, __FUNCTION__, __LINE__, __VA_ARGS__); \
if (safeAssertCallback) safeAssertCallback(__FUNCTION__, __LINE__, __VA_ARGS__);} \
sendToBugly(__FILE__, __FUNCTION__, __LINE__, __VA_ARGS__);
...
void SFLog(const char* file, const char* func, int line, NSString* fmt, ...)
{
    ...
    NSLog(@"错误信息:--%s|%s|%d|%@", file, func, line, [[[NSString alloc] initWithFormat:fmt arguments:args] autorelease]);
    va_end(args);
}

//把错误的信息上报bugly
void sendToBugly(const char* file, const char* func, int line, NSString* fmt, ...){
    //异常的名称
    NSString *exceptionName = @"对象数据异常";
    //异常的原因
    NSString *exceptionReason = [NSString stringWithFormat:@"文件:%s|方法名:%s|行数:%d|方法:%@",file, func, line, fmt];
    //异常的信息
...
{
#warning 这边极光里面不知道为什么用null 替换数据 ,不注释会注册不上
