...
#import "NSDate+BFAdd.h"

@implementation NSDate (BFAdd)
+ (NSString *)bf_stringFromDate:(NSDate *)date formatter:(NSString *)formatter {
    return bf_stringFromDate(date, formatter);
}

- (NSString *)bf_stringFromDate:(NSString *)formatter {
    return bf_stringFromDate(self, formatter);
}
...
- (BOOL)bf_isYesterday {
    //2014-05-01
    NSDate *nowDate = [[NSDate date] bf_dateWithYMD];
    
    //2014-04-30
    NSDate *selfDate = [self bf_dateWithYMD];
...
+ (NSDate *)bf_dateAfterDay:(NSDate *)date num:(int)num {
    return [date bf_dateByAddingDays:num];
}

/** 某天前几天 */
+ (NSDate *)bf_dateBeforeDay:(NSDate *)date num:(int)num {
    return [date bf_dateBySubtractingDays:num];
}

/** 减少dDays天 */
- (NSDate *)bf_dateBySubtractingDays:(NSInteger)dDays {
    return [self bf_dateByAddingDays:(dDays * -1)];
}
