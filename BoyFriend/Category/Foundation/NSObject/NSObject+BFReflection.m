...
#import "NSObject+BFReflection.h"
#import <objc/runtime.h>

@implementation NSObject (BFReflection)
...
{
    return [[self class] bf_propertyKeys];
}
+ (NSArray *)bf_propertyKeys {
...
{
    return [[self class] bf_propertiesInfo];
}
/**
...
+ (NSArray *)bf_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] bf_propertiesInfo];
...
-(NSArray*)bf_methodList{
    u_int               count;
    NSMutableArray *bf_methodList = [NSMutableArray array];
    Method *methods= class_copyMethodList([self class], &count);
    for (int i = 0; i < count ; i++)
    {
        SEL name = method_getName(methods[i]);
        NSString *strName = [NSString  stringWithCString:sel_getName(name) encoding:NSUTF8StringEncoding];
        [bf_methodList addObject:strName];
    }
    free(methods);
    return bf_methodList;
}
...
-(NSDictionary *)bf_protocolList{
    return [[self class]bf_protocolList];
}
+ (NSDictionary *)bf_protocolList
{
